...
// a struct which contains the first and last names found in the file. Each struct
// created will be added to a slice, and after all lines have been read from the
// file, your program will have a slice containing one struct for each line in the
// file. After reading all lines from the file, your program should iterate through
...
func main() {
	fmt.Println("Enter the file name containing collection of names :")
	var fileName string
	fmt.Scanln(&fileName)
	names := make([]Name, 0, 3)
	...
	for scanner.Scan() {
		...
		var pName Name
		pName.fname, pName.lname = s[0], s[1]
